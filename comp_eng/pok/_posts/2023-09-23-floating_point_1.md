...
FP8, які використовуються в задачах штучного інтелекту, конструюються по іншому, зокрема, часто, або без безмежностей та NaN, або під них виділено значно меншу кількість біт, ніж в стандарті IEEE 754. Приклад: ''[FP8 Formats for Deep Learning](https://arxiv.org/abs/2209.05433)'' згадує два типи:
...
Згідно [Вікі](https://en.wikipedia.org/wiki/IBM_7030_Stretch), найшвидший суперкомп'ютер 1961-64 років. 

Транзисторна машина, продано 9 одиниць. 64-бітовий процесор з паралельним ALU, до 16 мегабіт/256 кілослів[^KS7030] RAM, продуктивність 1.2 MIPS. В цілому, виявилася помітно повільнішою машиною, ніж планувалося. Однак, хоча вважається невдалою, на ній відпрацьовувалося багато інновацій -- конвеєр, попередня вибірка (prefetch) тощо. 

[^KS7030]: 64-бітових. Так, це схоже на 2 мегабайти, звичних нам, але тут байтом називали не тільки 8-бітові послідовності, вирівняні на 8.

Але найбільш цікаво не це:

- Команди 32-бітові або 64-бітові.
- [Core-пам'ять](https://en.wikipedia.org/wiki/Magnetic-core_memory) зберігає 72-бітні слова, з яких використовуються програмно **64**, а решта -- "check bits", які дозволяють випавляти однобітові помилки і виявляти всі двобітові. 
- Адреси 18-бітові.
- Числа з фіксованою крапкою -- змінної довжини. 
  - 1-64 біта двійкові та 1-16 цифр десяткові (у [BCD](https://en.wikipedia.org/wiki/Binary-coded_decimal) форматі), беззнакові, або у форматі знак-модуль. 
...
- Акумулятор цікавий -- 128+8-бітовий, має ліву 64-бітову частину, праву 64-бітову і ще, окремо, 8-бітовий регістр -- знак акумулятора.
  - Цитата [щодо цієї акумулюторної ISA](http://archive.computerhistory.org/resources/text/IBM/Stretch/pdfs/09-05/102634374.pdf): ''Universal accmulatos avoids misplacement of results''.

| ![](/comp_eng/pok/pics/IBM7030_int_data.png) |
|:--------------------------------------------------:|
| Приклад формату цілочисельних даних [з документації (стор. 17)](http://www.bitsavers.org/pdf/ibm/7030/22-6530-2_7030RefMan.pdf). Формат команд там ж, на на наступному рисунку, стор. 18. |

Floating point числа не настільки ''варіабельні''[^IBM3070n], але формат незвичний:

[^IBM3070n]: Через них цей текст і з'явився -- я шукав нетривіальні приклади представлення чисел з рухомою крапкою.
...
- Завжди починаються на границі слова RAM.
  - В документації обговорюється, що це довзоляє швидкісну паралельну обробку[^ES7030]. 
...
- Тобто, розмір 61/64 біти[^S14], які називають single precision, але є команди, які працюють з double precision -- двома такими словами.
...
- Заокруглення, як таке, не передбачено. 
  - В [документації (стор. 81-82)](http://www.bitsavers.org/pdf/ibm/7030/22-6530-2_7030RefMan.pdf) пропонується, для коректного заокруглення виконувати double precision операції та команду "Store rounded" в кінці, яка заокругляє, додаючи 1 до 49-го біта результату, та зберігаючи 48 біт.
- Інтерпретація нуля там весела -- їй ціла глава відділена (стор. 82-84).
- Є така ціквав штука -- Noisy mode.
...
[^ES7030]: Я зразу згадую вирівняні сучасні SIMD.

[^S14]: 64 біти в пам'яті, 61 -- фактичний розмір

| ![](/comp_eng/pok/pics/IBM7030_FP_data.png) |
|:--------------------------------------------------:|
| Формат чисел з рухомою крапкою одинарної точності, [з документації (стор. 17 та 78)](http://www.bitsavers.org/pdf/ibm/7030/22-6530-2_7030RefMan.pdf).  |


Детальніше див.:

- [IBM 7030 Data processing system: Reference manual](http://www.bitsavers.org/pdf/ibm/7030/22-6530-2_7030RefMan.pdf)
- [IBM Stretch (7030) -- Aggressive Uniprocessor Parallelism](https://people.computing.clemson.edu/~mark/stretch.html)
- [Стаття на вікі](https://en.wikipedia.org/wiki/IBM_7030_Stretch)
