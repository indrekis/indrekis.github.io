...
> Обережно -- далі 20 Мб  фотографій, більшість з яких, на жаль, неякісні.
...
|:--------------------------------------------------:|
| Compaq Portable III, 286, [фото з журналу "Australian Personal Computer", Volume 8, Number 6, June 1987](https://archive.org/details/1987.06-australian-personal-computer/page/38/mode/1up?q=compaq). Мій зараз помітно жовтіший... Зверніть увагу на клавіатуру. |
...
- ISA Expansions Slot із двома 16-бітовими слотами ([\$199](https://archive.org/details/family-computing-53/page/n53/mode/1up?q=compaq+portable&view=theater)).
...
- Клавіатура приємна, достатньо стандартна в 386, але дивна в 286. 
  - Зокрема, в 286, Esc -- на малій цифровій, над Home, а Enter на ній немає, замість нього -- великий плюс.
...
|:--------------------------------------------------:|
| Конфігурація 286, використовується MDA. |

| ![](/retrocomputing/ibm_pc_compat/pics/CompaqPortable3/conf_386_1.jpg) |
|:--------------------------------------------------:|
| Конфігурація 386, використовується CGA. |
...
- Expander unit з 40 Мб носієм -- змінним картриджем з магнітною стрічкою, для бекапу. 
- 360 Кб 3.5" FDD на заміну (тому 386-й з 3.5" не такий вже "анахронічний", та й 1.44Мб-варіант згадується в "COMPAQ 80286-Based Products Technical Reference Guide").
...
|:--------------------------------------------------:|
| Діаграма можливих оновлень пам'яті для 386 варіанту, [взято тут](https://minuszerodegrees.net/manuals/Compaq/Compaq%20Portable%20386%20-%20Detailed%20Features%20and%20Specifications%20(1987).pdf). Зауважте, потребує додаткових плат-адаптерів. |
...
- AUTO -- 12 в нормі, і 8, коли активний дисковод. Документація стверджує, що це потрібно деяким програмам, підозрюю -- це був якийсь захист від копіювання.
...
[^FC2]: Такі назви, ймовірно, через те, що в [Compaq Portable II](https://minuszerodegrees.net/manuals/Compaq/Compaq%20Portable_Plus_286%20-%20Maintenance%20and%20Service%20Guide.pdf) були режими FAST (8 MHz) та COMMON (6 MHz). Наші в режимі COMMON працювати не хочуть. 
...
- 286-й пищить раз під час завантаження -- це мало б означати AUTO режим, однак, частота 8 МГц. 
  ...
  - Ймовірно, причина -- нетипове розташування джамперів після ремонту, не став розбирати, щоб чого не зламати.
...
> Розбиратися довелося довго. Спочатку пробував за допомогою переривань -- API BIOS та портів, з допомогою документації та статей в Інеті, нічого не виходило. Успіху вдалося досягнути аж після застосування MODE.COM з Compaq DOS 3.31, дизасемлювання його та, потім, трасування 16h-го переривання. 
> Кумедно, хоча часту правильних дій я робив і раніше, очевидно, щось впускав, тому не зауважив результату.
...
[^DV2]: Найбільше використовую саме цю версію, за сукупністю причин, тому вона на HDD.

[^SVC]: Утиліти DOS відмовляються працювати з "чужою" версією самої операційної системи, але SETVER.EXE дозволяє повідомляти, вказаним за іменем -- без шляху абощо, виконавчим файлам потрібну їм версію -- в надії, що все працюватиме. 
...
[^P86]: На  Можливо, є сенс поекспериментувати з MODE чи повним кодом int 16...
...
- [Int 16/AH=F1h](http://www.ctyme.com/intr/rb-1918.htm) -- "Compaq 386 and newer - READ CURRENT CPU SPEED"
  - Що не зовсім правда, Portable III їх ніби теж підтримує, принаймні, помилки не видає, і повернене значення -- відповідає стану справ, 8 МГц.
...
|:--------------------------------------------------:|
| Змінюємо швидкодію на 286, за допомогою портів. Зміна швидкодії контролювалася також CheckIt.|

Трасування обробника int 16h показує, що **386-й для зміни частоти**:

- В FAST:
  - Читає значення з порта 0x86, але, фактично, його ігнорує, записує туди 0x27.
  - Забороняє переривання.
  - Виводить в 0x4B 0x92.
  - Виводить в 0x4A 0x27 (перевіряючи якусь умову перед тим).
  - Дозволяє переривання.
- В HIGH (опускаючи маніпуляцію перериваннями):
...
> Ілюстрація, хай -- банальна, що використання переривань BIOS дає вищий рівень сумісності, ніж портів безпосередньо. 

[^NS64]: Нестандартне використання -- в IBM PC це порт "keyboard controller read status" (читання)/"keyboard controller input buffer" (запис), але й інші машини його ще для чогось використовують (PORTS.A згадує Amstrad/Schneider PC1512, AMI BIOS тощо)
...
А ось цей [старіший API](http://www.ctyme.com/intr/rb-1920.htm), певне, не працює: 

```nasm
  ; [Cтаріший API: ](http://www.ctyme.com/intr/rb-1920.htm)
...
За ручного задання швидкодії, 1 (мінімальна), давала 597 Dhrystones на 386, 2 -- 664, 3 --, 746, 4 -- 814, 10 -- 1280,  а максимальна, 50 -- відповідала HIGH. Повну табляцію полінувався робити. :-) На 286 вона повністю ігнорується -- в тому числі, й MODE не бачить змін.

| ![](/retrocomputing/ibm_pc_compat/pics/CompaqPortable3/min_386_speed.jpg) |
|:--------------------------------------------------:|
| Мінімальна швидкодія для 386. Вибачте за якість фото! |
...
|Ціна в доларах 2023  | [\$5133](https://books.google.com.ua/books?id=BT8EAAAAMBAJ&lpg=PA61&dq=Amstrad%201640&pg=PA61#v=onepage&q=Amstrad%201640&f=false)      | \$4332 | $15674 | -"- | $27026 |
...
Мій 286-й варіант у мене вмикається як MDA. 
...
І на 286, і на 386, завжди 0x42 -- 0100 0010b, 640x400 / CGA, очікувано.

Те ж з підключеною VGA.
...
Також, в тій статті описано керування шрифтами, зовнішнім дисплеєм і т.д.
...
>
> Якщо так "завмерло" через переривання -- вивід у порт значення за замовчуванням виправляє ситуацію. Це підтвердилося дизасемблюванням та трасуванням обробника.
...
3. Оновлює байт за адресою 410h.
   - Власне, ось це я не робив до того, як почав дизасемблювати.  
4. Змінює відеорежим, викликом [Int 10/AH=00h](http://www.ctyme.com/intr/rb-0069.htm)

Останнім штрихом було те, що в байті за адресою 410h, де зберігається "[список апаратури](https://stanislavs.org/helppc/bios_data_area.html)"[^LGT], його ж повертає в AL виклик [int 11h](http://www.ctyme.com/intr/rb-0575.htm). Його біти 4 і 5 містять код відеорежиму, на який, очевидно, покладається BIOS:
...
Асемблював TASM версії 3.1, скориставшись ключем /t для створення .COM-файлу:
...
> Під QEMM ці утилітки теж не працюють, навіть прапорець /X для CGA i MDA не допоміг. 
...
VGA картка -- в розширювачі, Trident 9016×2/4 LT2 (1991), 16-бітова ISA. З QEMM тестувався лише CGA -- режим переключити утруднено, коли він активний, а ось швидкість процесора зберігається за гарячого перевантаження. 

Деякі спостереження з таблиці: 

- Цікаво, що зростання продуктивності MDA Direct-доступу для 286 помітно більше, ніж якби вона зростала лінійно з частотою (4078*1.5 = 6117). Не дуже розумію, що могло б це спричинити.
...
|:------------------------------------------------:|
| Конфігуратор від Compaq - 2. Запущено на 386. Одна із смужок зверху "справжня", інша -- дефект.|
...
|:------------------------------------------------:|
| Вивід INSPECT\.EXE, запущено на 386.|
...
|:------------------------------------------------:|
| Вивід INSPECT\.EXE -- вміст CMOS, запущено на 386. Видно процес оновлення екрану|
...
46 FF 24 00 22 00 07 15   01 24 26 02 40 80 00 00 
40 00 F0 00 21 80 02 00   0C 2D 00 00 00 00 00 00 
00 00 00 00 33 00 00 00   00 08 00 00 0F 85 02 DB 
00 0C 20 80 00 00 00 00   00 00 00 00 00 00 00 00   
```
...
- оригінальні образи з Winworldpc: FASTART.IMG, OPER.IMG, STARTUP.IMG;
...
Підключити його вимагає помітного механічного зусилля та акуратності -- пластмасові фіксатори ламаються. Якщо буде потрібно (ці модулі доволі рідкісні) -- пишіть, спробую детальніше пояснити процес. Але ідейно процес простий -- відсунути захисну засувку ззаду, під'єднати до роз'єму та зафіксувати, підсунувши планку внизу розширювача. 
...
Після підключення зовнішньої відеокарти хоче, щоб запустили SETUP, хоча ніби від цього нічого не змінюється. Ретельно не тестував. Щоб почав юзати зовнішню відеокарту -- потрібно запустити сетап. Можна нічого не міняти.
...
Від вибору, за допомогою modecmpq, зараз активний CGA чи MDA, поведінка не змінювалася. Можливо, причина в розташуванні джамерів, і MDA за замовчуванням якраз дозволяє 286-му нормально працювати з зовнішньою VGA. Якісь такі натяки траплялися мені в документації...

# Досвід експлуатації

## Обидва

Volkov Commander працює звично. Dos Navivator мигає, за стандартної палітри і налаштувань відображення дисплею за замовчуванням. Звичайно, потрібно обрати 
...
|:-----------------------------------------------:|
| Волков коммандер, запущено на 286. |

| ![](/retrocomputing/ibm_pc_compat/pics/CompaqPortable3/dn_1.jpg) |
|:-----------------------------------------------:|
| Дос навігатор 1.51, запущено на 286. |
...
|:-----------------------------------------------:|
| Turbo Basic 1.0 (1987), запущено на 286, але працює і на 286. |
...
- З Windows 3.11 подружити його не вдалося -- трішки подробиць далі, але Windows 2.xx проблем з ним не має.
- Quickboot не працює. 

Враховуючи, що раніше з такими проблемами не стикався, підозрюю -- це наслідок особливостей цієї машини. 

Також, спробував CEMM -- все ж, [перший менеджер пам'яті для 386, що використовував v86](https://www.os2museum.com/wp/original-cemm-unearthed/), та ще й від Compaq. Ну, він працює -- але, це лише емулятор EMS, тому не заглиблювався поки. Короткий підсумок:
...
Результати наступні, за умови присутності, мінімум, SETVER, SMARTDRV, DOSKEY, CTMOUSE (детальніше див. різні варіанти конфігурації нижче), MS DOS 6.22:
...
|:-----------------------------------------------:|
| А ось QEMM (тут -- + HIMEM.SYS) з 4Мб "робить" 3 Мб Extended і 3.6 Мб Expanded. :-) |
...
Вікно DOS не створює -- каже 386 System display type mismatch. 

Delpi-1 вдалося заінсталювати і навіть трішки покористуватися, але, насправді, на CGA вона майже не придатна до використання -- дрібні літери, у шрифтах губляться штрихи і важко роздивитися.  А ось з plasma-драйвером, цілком гарно виглядає, хоч і гальмує. 
...
|:-----------------------------------------------:|
| Total Commander 5.51, plasma. Цікаво, як змінилося візуальне представлення через зміну роздільної здатності з 640x200 на 640x400. | 
...
Чомусь, природний ніби, "Compaq Portable Driver" не працює (ні вбудований, ні той, що використав для Windows 3.11 -- навіть коли розархівував його файли, за допомогою EXPAND.EXE), використовую CGA, що постачався з Windows, на вбудованому дисплеї та VGA на зовнішньому. 
...
|:-----------------------------------------------:|
| Windows 3.00 на 286, з AST Advantage (4 Мб) та VGA картою і відповідним драйвером. Захищений режим 286-го використано. |

| ![](/retrocomputing/ibm_pc_compat/pics/CompaqPortable3/win300_cga_on_vga_1.jpg) |
|:-----------------------------------------------:|
| Windows 3.00 на 286,VGA карта, але дарйверй CGA. Він -- чорно-білий, не підтримує кольорів. |
